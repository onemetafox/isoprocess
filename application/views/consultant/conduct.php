...
<script type="text/javascript">
	function close_control(id){
		bootbox.confirm("Are you sure?", function(result) {
			if (result) {
				var params = {
					'ids' : id
				};
				$.post('<?php echo base_url(); ?>index.php/Consultant/close_control', params, function(data, status){
					if (status == "success") {
						new PNotify({
							title: 'Success',
				...
				window.location.href = "<?php echo base_url();?>index.php/consultant/conduct";
			}
		});
	}
</script>
...
<script type="text/javascript">
	...
	function onActiveOrInactive(id, active) {
		msg = 'Do you really set ' + (active == 1 ? 'active?' : 'inactive?');
		bootbox.confirm(msg, function(result){
			if (result)
				location.href = '<?= base_url("consultant/control_active") ?>/' + id + '?active=' + active;
		});
	}
</script>
