...
class Auth extends BaseController//CI_Controller
{
	...
	public function login()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (isset($usertype)) {
				if ($usertype == 'admin') {
					$data = $this->Authmodel->admin_login($username, $password);
					if ($data) {
						$data1 = array(
							'admin_id' => $data->id,
							'user_type' => $usertype,
				...
				if (($usertype == 'Lead Auditor') || ($usertype == 'Auditor') || ($usertype == 'Process Owner') || ($usertype == 'Auditee')) {
					$data = $this->Authmodel->employee_login($username, $password, $usertype);
					if ($data) {
						$data1 = array(
				...
				if ($usertype == 'Consultant') {
					$data   = $this->Authmodel->consultant_login($username, $password);
	...
	public function register()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($data_success) {
				...
				$email_tempU['message'] = str_replace("{verification_link}", $verificaiton_link, 
					$email_tempU['message']);
	...
	public function add_purchase($plan_id = NULL)
	{
		...
		if ($plan_id == '0') {
			...
			if($plan_id == '1'){
				...
				$item_data = [
					'description'=>$plandata->name.' Membership Payment',
	...
	public function forgot_pass_send_link()
	{
		...
		if ($email != '') {
			...
			if (!empty($very1) || !empty($very2) || !empty($very3)) {
				...
				// $this->sendemail($email, 'Email Verification', $email_tempU['message'], $email_tempU['subject']);
				$result = $this->send_mail($email, 'Forgot Password', $email_tempF['message'], $email_tempF['subject']);
				//-------------------------------------------------------
	...
	public function resetPassword ($recovery_link = '') {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$update = $this->db->update('consultant', $up);
